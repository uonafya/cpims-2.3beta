 ...
 */
 ...
 CREATE MATERIALIZED VIEW data_quality_view AS
SELECT ovc_registration.id AS ovc_registration_id,
   ...
   FROM ovc_registration
     LEFT JOIN reg_person ON ovc_registration.person_id = reg_person.id
     LEFT JOIN reg_org_unit ON ovc_registration.child_cbo_id = reg_org_unit.id
	 WITH DATA;
